...
class ChangeSet9Tests {

    private val changeSets = loadChangeSets(loadChangeSetsContent()).filter { changeSet -> changeSet.id <= 9 }
    ...
    fun `should add child default value`() {
        ...
        val scheme = Scheme.newBuilder().apply {
            ...
            putNodes(
                "NODE_ID_2",
                EquipmentNode.newBuilder().apply {
                    libEquipmentId = EquipmentLibId.SHORT_CIRCUIT_1PH
                    putFields("SUBSTATION", "Awesome substation id")
                }.build()
            )
            putNodes(
